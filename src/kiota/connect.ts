...
class WritableStreamWrapper implements rpc.RAL.WritableStream {
  ...
  public write(
    ...
    encoding?: rpc.RAL.MessageBufferEncoding,
  ): Promise<void> {
...
export default async function connectToKiota<T>(
  callback: (connection: rpc.MessageConnection) => Promise<T | undefined>,
  ...
  const inputReader = new rpc.ReadableStreamMessageReader(
    new ReadableStreamWrapper(childProcess.stdout),
  );
  const outputWriter = new rpc.WriteableStreamMessageWriter(
    new WritableStreamWrapper(childProcess.stdin),
  );
